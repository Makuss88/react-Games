...
const Header = ({ moves, bestScore, handleRestart, points }) => {
  ...
  const iconBad = <img src={icon1} alt="badIcon" />
  const iconGood = <img src={icon2} alt="goodIcon" />
  ...
  useEffect(() => {
    if (points === 9) {
      if (moves <= 16) {
        ...
        setStar(<span>{iconGood} {iconGood} <img src={icon1} alt="badIcon" /></span>)
      } else {
        setStar(<span>{iconGood} <img src={icon1} alt="badIcon" /> <img src={icon1} alt="badIcon" /></span>)
      }
    } else {
      setStar(<span>{iconBad} <img src={icon1} alt="badIcon" /> <img src={icon1} alt="badIcon" /></span>)
    }


  }, [points, moves, iconBad, iconGood])
